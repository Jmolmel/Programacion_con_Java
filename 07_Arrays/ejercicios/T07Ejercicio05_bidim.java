public class T07Ejercicio05_bidim {
  public static void main(String[] args) {
    ...
    for (int i = 0; i < numero.length; i++) {
      for (int j = 0; j < numero[i].length; j++) {
        if (numero[i][j] == maximo) {
          System.out.println("El número MAXIMO es: " + numero[i][j] + " en la fila " + (i + 1) + " columna " + (j + 1));
        } else if (numero[i][j] == minimo) {
          System.out.println("El número MINIMO es: " + numero[i][j] + " en la fila " + (i + 1) + " columna " + (j + 1));
        }
      }
    }
  }
